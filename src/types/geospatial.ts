...
export type WithGeoSpatialQueryOperator<Field> = Field extends (TsGeoJSON<TSGeoJSONTypes> | TsLegacyCoordinates)
  ? {
    $geoIntersects?: { $geometry: TsGeoJSON<"Polygon"> | TsGeoJSON<"MultiPolygon"> }
    $geoWithin?: { $geometry: TsGeoJSON<"Polygon"> | TsGeoJSON<"MultiPolygon"> } | WithGeoWithInPointOperators<Field>
  } & WithNearOperators<Field>
  : {}

export type WithNearOperators<Field> = Field extends (TsGeoJSON<"Point"> | TsLegacyCoordinates)
  ? {
...
export type WithGeoWithInPointOperators<Field> = Field extends (TsGeoJSON<"Point"> | TsLegacyCoordinates)
  ? (
    | { $box: TsGeoBox }
    ...
    | (Field extends TsLegacyCoordinates
      ? (
        | { $center: TsGeoCenter }
        | { $polygon: TsGeoLegacyPolygonCoords }
      )
      : never)
  )
  : never
...
export type TsGeoLegacyPolygonCoords = [TsLegacyCoordinates, TsLegacyCoordinates, TsLegacyCoordinates, ...TsLegacyCoordinates[]]